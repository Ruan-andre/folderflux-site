...
interface DownloadButtonProps {
  ...
  getDownloadUrl?: (os: "windows" | "linux" | "other") => DownloadUrlFactoryType;
}

export default function DownloadButton({
  href = "#download",
  fullWidth = false,
  children = "Baixar Gratuitamente",
  sx = {},
  size = "large",
  autoDetectOS = false,
  getDownloadUrl,
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLButtonElement>) => {
      ...
      const { url, status, message } = getDownloadUrl(os);
      if (status === "ok" && url) {
        window.location.href = url;
      } else if (status === "newVersion") {
        alert(message);
      } else {
